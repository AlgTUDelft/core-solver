...
/**
 * Assigns the rewards to the agent with the highest degree of dependency
 * 
 * @author Joris Scharpff
 */
public class CRGRewardHeurHighestDegree extends CRGRewardHeuristic {
	...
	public Map<Agent, CRGRewards> assign( Collection<Agent> agents, List<CRGReward> rewards ) {
		...
		// assign to the agent with the highest degree in the scope
		for( CRGReward r : rewards ) {
			// find the highest degree in its scope
			Agent ag = null;
			for( Agent a : r.getScope( ) )
